                        ...
                        <div style={{backgroundImage: `url(https://nnexsus.net/${el.imageref})`, backgroundSize: 'cover', borderRadius: '10px', border: 'solid var(--accentThemeDarker) 2px'}}>
                            <div className='p-project'>
                                ...
                                <div className='past-tags'>
                                    <h4 className='past-h4' style={{backgroundColor: `${initcol[el.initresult]}`}}>Outcome: {initresult[el.initresult]} - {el.initresult}/5</h4>
                                    <h4 className='past-yeartag'>{datef[0]}</h4>
                                </div>
                                <p className='past-desc' id={`pastchase-${ind}`} style={{WebkitLineClamp: 2}}>{el.alttext}</p>
                                <button id={`expandpast-${ind}`} className='past-expand' onClick={() => expand(ind)}>Expand <img style={{marginLeft: '3px'}} width={'24px'} height={'24px'} alt='decor' src='/images/bgs/Book.ico' /></button>
                            </div>
                        </div>
                    </div>
                )
...
//<img style={{cursor: 'pointer'}} title='click to expand' alt='project preview' className='webimg' loading={'lazy'} src={`${el.imageref}`} width="90%" height={'100%'} />