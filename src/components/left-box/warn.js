...
const Warn = () => {
    ...
    const getWarnings = () => {
        axios.get("https://api.weather.gov/alerts/active?status=actual&message_type=alert&region_type=land&urgency=Immediate,Expected&severity=Extreme,Severe&certainty=Observed,Likely&limit=250").then((res) => {
            setWarning(res.data)
    ...
    const FilterCategories = {
        ...
        "all": ["Tornado Warning",
            ...
            "Avalanche Watch",
            "Blizzard Warning",
            "Blizzard Watch","Red Flag Warning",
            "Fire Warning",
            "Fire Weather Watch",
            "Extreme Fire Danger",
            ...
            "Hard Freeze Warning",
            "Hard Freeze Watch",
            "Brisk Wind Advisory","Air Quality Alert",
            "Ashfall Advisory",
            "Ashfall Warning",
            "Dense Fog Advisory",
            "Dense Smoke Advisory",
            ...
            "Dust Storm Warning",
            "Blowing Dust Advisory",
            "Blowing Dust Warning","Tsunami Advisory",
            "Tsunami Warning",
            "Tsunami Watch",
            "Rip Current Statement"]
    }
            ...
            {warning !== null ?
                <>
                    <div className="thick-scroller" style={{display: 'flex', overflowX: 'scroll', marginTop: '8px'}}>
                        <button className="filter-button" onClick={() => setFilter("all")}><img src="/images/16icons/control.png" alt="decor" width={'32px'} height={'32px'} />No Filter</button>
                    ...
                    <div className="warncontainer">
                        {warning.features.map((warn, ind) => {
                            ...
                            return (
                                <div style={{backgroundColor: `${bgcol}`, border: 'inset 3px', margin: '10px', padding: '2px', borderRadius: '2px', outline: 'solid black 1px', outlineOffset: '1px'}} key={warn.id}>
                                    ...
                                    <button id={`expandwarn-${ind}`} style={{paddingLeft: '4px'}} className='past-expand' onClick={() => expand(ind)}>Expand <img loading='lazy' style={{marginLeft: '3px'}} width={'24px'} height={'24px'} alt='decor' src='/images/bgs/Book.ico' /></button>
                                    <div>
                                        {svg ?
                                            <svg viewBox={`${lon - 0.703125} ${lat - 0.703125} 1.40625 1.40625`} width="100%" height="100%">
                                                <defs>
                                                    <pattern id={`img1-${ind}`} patternUnits="userSpaceOnUse" width={`${(1.40625 / 8)}`} height={`${(1.40625 / 8)}`}>
                                                ...
                                                <path style={{transform: `translate(${((lon - 0.703125) + (lon + 1.40625))}px, 0px) scale(-1, 1)`}} fillRule="evenodd" fill={`url(#img1-${ind})`} strokeLinecap="round" strokeLinejoin="round" stroke={`${WarnCategories[`${warntype}`]?.color}`} strokeWidth="0.0075" 
                                                d={`${path}`}>
                                                </path>
                                            </svg>
                                        : null}
                        ...
                        {warning.features.length == 0 ? 
                            <div style={{backgroundColor: 'lightblue', border: 'inset 3px', margin: '10px'}}>
                                <h2 style={{display: 'flex', alignItems: 'center'}}><img loading='lazy' alt="decor" height={'32px'} width={'32px'} src={`/images/warns/heat.png`} />No warnings.</h2>
                                <p>No warnings! If you're using a type filter, try another category.</p>
                            </div>
                        :null}
                    </div>
                </>
