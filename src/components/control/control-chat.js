    ...
    const InputMessage = () => {
        ...
        const sendMessage = () => {
            if (newmessage !== '' && newmessage !== null) {
                ...
                socket.emit('send_message', {'username': `${username}`, 'message': newmessage, '__createdtime__': __createdtime__, 'effects': JSON.stringify(effects), 'key': apikey})
                setNewmessage('')
            }
        }
        ...
        return (
            <div className="message-write">
                <div className="message-input" style={{display: 'flex', margin: '3px', outline: '1px black solid'}}>
                    ...
                    <input id="message-input" style={{width: 'calc(100% - 12px)', height: '35px', backgroundImage: 'url(/images/bgs/BlackThatch.png)', border: 'inset 3px', borderRight: 'none', fontFamily: 'ms ui gothic'}} type="text" title="Username or title" placeholder="Username/Title" value={username} onChange={(e) => setUsername(e.currentTarget.value)} />
        ...
        return (
            <div className="message-container-wrapper" ref={messageContainer}>
                {messages.map((el, ind) => {
                    ...
                    const texts = removeTextBetweenTags(el.text)?.split(",")
                    const imgs = extractTextBetweenTags(el.text)
                    const reference = filterById(streams, extractTextBetweenReferences(el.text).toString().substring(1))
                    const effects = el.effects !== undefined ? JSON.parse(el?.effects) : {}
                    const color = effects?.color ? effects.color : 'lightgreen'
                    const font = effects?.font ? effects.font : 'ms ui gothic'
                    const decor = effects?.effect ? `${effects.effect + " " + effects.effectColor}` : 'none'
